...
/* Wrapper that applies the standard hero entrance animation */
function FadeIn({ delay, className, children }) {
...
function HeroSection() {
  return (
    <section className="relative h-screen overflow-hidden bg-black">
      ...
      {/* Background video — 120% wide/tall, top-anchored, no overlay */}
      <FadingVideo
        src={HERO_VIDEO}
      ...
      {/* z-10 content layer: Navbar → centred content → Partners */}
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 flex flex-col items-center justify-center pt-24 px-4 text-center">
          ...
          <FadeIn delay={0.4} className="flex items-center">
            <div className="liquid-glass rounded-full flex items-center gap-0 px-1.5 py-1.5">
              <span className="bg-white text-black rounded-full px-3 py-1 text-xs font-semibold font-body leading-none">
                New
              </span>
              <span className="text-sm text-white/90 font-body pr-3 pl-2">
                Maiden Crewed Voyage to Mars Arrives 2026
              </span>
            </div>
          </FadeIn>

          {/* Headline — BlurText, no outer FadeIn delay (BlurText handles its own timing) */}
          <div className="mt-4">
            <BlurText
        ...
        {/* Partners — delay 1.4s, pinned to bottom */}
